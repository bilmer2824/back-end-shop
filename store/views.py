...
from .models import Category, Product, Review, FavouriteProducts, Mail, Customer
...
class CategoryView(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context['title'] = f"{main_category.title} kategoriyasi mahsulotlari"
        return context
...
def user_login(request):
    ...
    if form.is_valid():
        ...
        messages.error(request, "Foydalanuvchi ismi yoki parol notog'ri!")
...
def register(request):
    ...
    if form.is_valid():
        form.save()
        messages.success(request, "Akkunt muvaffaqiyatli ochildi! akkountga kiring!")
    else:
...
def to_cart(request, product_id, action):
    ...
    if request.user.is_authenticated:
        ...
        messages.error(request, "Xaridni davom ettirish uchun, Authetikatsiyadan o'ting")
        return redirect('login_registration')


def checkout(request):
    """Buyurtma qilish sahifasi"""
    cart_info = get_cart_data(request)
    context = {
        'cart_total_quantity': cart_info['cart_total_quantity'],
        'order': cart_info['order'],
...
def create_checkout_sessions(request):
    ...
    if request.method == 'POST':
        """Bu joyda foydalanuvchilarni ma'lumotlarini saqlaymiz"""
        user_cart = CartForAuthenticatedUser(request)
        cart_info = user_cart.get_cart_into()
        ...
        if shipping_form.is_valid():
            ...
            address.order = user_cart.get_cart_into()['order']
            address.save()
...
def successPayment(request):
    user_cart = CartForAuthenticatedUser(request)
    user_cart.clear()
    messages.success(request, "To'lovingiz muvaffaqiyatli amalga oshirildi!")
...
def clear_cart(request):
    user_cart = CartForAuthenticatedUser(request)
    order = user_cart.get_cart_into()['order']
